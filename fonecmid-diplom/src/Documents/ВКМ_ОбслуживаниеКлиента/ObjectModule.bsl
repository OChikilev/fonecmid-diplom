...
КонецЦикла;
	...
	Движение.ВидДвижения = ВидДвиженияНакопления.Приход;
	Движение.СуммаКОплате = ДанныеСотрудникаОтРабот.Оклад * ПлановоеКоличествоЧасов + (ДанныеСотрудникаОтРабот.Оклад * ПлановоеКоличествоЧасов * ДанныеСотрудникаОтРабот.ПроцентОтРабот / 100);
...
Функция ПолучитьДанныеСотрудникаОтРабот(Сотрудник)
	...
	КонецЕсли;
		...
		Если ДанныеСотрудникаОтРабот.Оклад = 0 Тогда
			Отказ = Истина;
			ОбщегоНазначения.СообщитьПользователю("Не найден оклад сотрудника " + Сотрудник, ЭтотОбъект, "Сотрудник", "Объект", Отказ);
		КонецЕсли;
